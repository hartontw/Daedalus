class Command {
    ...
    async send(content, first = true) {
        try {
            ...
            if (this.args.remains || !first)
                return this.message.channel ? await this.message.channel.send(content) : undefined;

            return !this.message.system ? await this.message.reply(content) : undefined;
        } catch (error) {
            return this.error(error);
        }
    }
